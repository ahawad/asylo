...
class MutexGuarded {
   ...
   public:
    ...
    std::pair<bool, LockView<T>> LockWhenWithTimeout(
        ...
        ABSL_LOCKS_EXCLUDED(mu_) {
        auto condition_function = [this, cond] { return cond(value_); };
        bool cond_is_true = mu_.LockWhenWithTimeout(
...
template <typename T>
class ABSL_SCOPED_LOCKABLE LockView {
   public:
    LockView() = delete;
    ...
    bool AwaitWithTimeout(std::function<bool(const T &)> cond,
        ...
        auto condition_function = [this, cond] { return cond(*value_); };
        return mu_->AwaitWithTimeout(absl::Condition(&condition_function),
                                     timeout);
    }
...
template <typename T>
class ABSL_SCOPED_LOCKABLE ReaderLockView {
   public:
    ...
    ~ReaderLockView() ABSL_UNLOCK_FUNCTION() {
        if (mu_ != nullptr) {
            mu_->ReaderUnlock();
        }
    }
